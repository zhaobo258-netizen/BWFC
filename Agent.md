...
Scripts/run_tests.sh        # 全部测试（当前 631 例 79 套件，必须全绿；BWFX_IT_MEDIA=1 加真实媒体探针）
...
1. `v0.2.2 (6)` 支持一次人工指认后批量回填同组发言、自动写入永久声纹并回查本场历史片段，人工背景与有证据的表达沟通画像可跨会议复用。本地 631 例/79 套件全绿；真实多人录音与 OpenAI 兼容分人服务仍需现场验收。

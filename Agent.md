...
Scripts/run_tests.sh        # 全部测试（当前 449 例 66 套件，必须全绿；BWFX_IT_MEDIA=1 加真实媒体探针）
...
Core/
  ...
  Analysis/     AIProviderRegistry（Kimi/OpenAI-compatible）、
                PromptRegistry、ProjectAIOrchestrator、FinalReportAgent，
...
- 所有 AI 文本生成走 `AITextGenerationServing` / `AIProviderRegistry`；每次请求只读一次配置快照。设置修改只影响下一次请求，不能取消或换掉正在执行的请求。
- `PromptRegistry` 是共享安全红线、场景规则、任务 Prompt、固定 JSON 合同与 Prompt Version 的唯一入口；普通设置页不开放原始 Prompt。
- 完整总结只读 `ProjectAIContext` 的分析证据账本和对应片段，不读用户笔记、不查互联网/MCP；开花外部 MCP 只收最长 24 字短检索词。
...
- Kimi、OpenAI-compatible 分析、分人和每个 MCP 凭证**互不外借**；某 Provider 401 只暂停它自己。旧 `openai` 条目启动时自动迁移到 `kimi`。
...
14. **完整总结与实时分析必须分开持久化**：`finalReportSnapshots` 最多三版，输入指纹只覆盖场景、说话人及最终/人工修订片段；新增 Project 字段同步登记字段所有权和工作台刷新清单，不升级 schemaVersion。
...
1. 设置中心、完整总结和 AI 编排改造当前只在工作树中实施，**尚未 commit、push 或覆盖安装**；`~/Applications/BangWoFenXi.app` 不能当作本轮已生效。本轮 449 例/66 套件全绿，调试 App 已打包并通过签名校验；原生界面已验证当前屏幕可提供的 1189×768 和 960×640 内容最小尺寸，1440×900 仍只有布局单测覆盖。
...
5. 完整总结真实 Kimi/OpenAI-compatible 请求、真实短录音/导入、重开与证据跳转仍需本机现场验收；不得使用真实客户录音做探针。

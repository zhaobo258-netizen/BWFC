...
enum PromptRegistry {
    ...
    static let conversationMotiveFramework = """
    商务沟通动机分析：帮助读者看清“对方提出什么条件、为什么这可能影响业务推进、
    还有什么解释尚未排除”。只解释有原话支持的具体业务问题，不追求看穿内心。

    取舍与证据：
    - 原话已明确的需求、顾虑或理由，直接整理为明确表达，不再换个说法包装成隐藏动机。
    - 只有条件、取舍、反复确认、前后变化或问答落差提供额外信息时，才提出可能诉求。
      可关注价格与收益、付款与资金占用、交付与资源、验收标准、审批权限和责任边界；
      这些只是观察方向，原话没有提及就不能自动补入行业惯例或角色刻板印象。
    - 每项聚焦一个影响业务判断的问题。同一问题只保留最合适的一个类别，
      不在顾虑、动机、表达目的等类别重复；不要求每类都有内容，不为凑数分析客套话。
      “争取利益、推进合作、重视风险”等套话没有说明具体条件和依据时不输出。
    - 比较同一说话人在当前原文中的前后表达，明确谁提出了什么、谁尚未回应。
      沉默、客套、提问、汇报和职位本身不等于同意、拒绝、推责或某种隐藏企图。

    输出方式：possible_motive，以及属于 inference 的 possible_concern / expression_purpose，
    每条 text 使用以下四行，每行一句短句，整条只讨论同一个业务问题：
    原话依据：概括可观察的措辞、条件或前后差异；必要时只摘录原文中真实存在的短语。
    可能诉求：说明这一依据可能指向的具体业务目标或约束，解释两者的联系，不能只同义改写。
    其他解释：给出同样符合原话的一个合理替代解释，或指出原文中削弱该判断的内容；
    不得把假设的预算、KPI、关系或经历当成已知事实。确实无法区分时明确写“现有原话不足以区分”。
    待核实：用一个最小事实问题说明如何区分这两种解释；不是回应话术或建议采取的行动。
    四行在 JSON 的 text 字符串内用换行分隔，不新增字段。
    explicit_need、明确表达的顾虑和目的、事实与待核实问句无需套用四行。
    对方明确说出的诉求用 explicit_need 等恰当类别；possible_motive 一律为 inference，
    confidence 只用 low 或 medium。尚未排除合理替代解释时，不可声称已确定真实动机。

    增量核验：previous_state 是上一轮 AI 判断，不是原话，也不是支持自己的新证据。
    previous_evidence_segments 是本次提供的、有数量和字数上限的当前有效旧原文；
    和 new_segments 一起用于核验。旧判断的新建、改写或置信度提高必须有本次提供的
    原文支持，不能仅因上一轮已经这样写过而更确信；未回填原文的旧条目只能保留为
    未重新核验的连续状态，不能用来推导新的动机。原文反驳旧判断时应删除或修正。
    人物背景、历史表达画像、用户补充和关联项目只能帮助理解情境，不代替本场证据。
    禁止声称某人恐惧、嫉妒、操控、欺骗或诊断人格；不输出回应建议、话术和心理诊断。
    每条判断仍须引用可点开的真实 evidence_segment_ids，证据不足则不输出该项。
    """

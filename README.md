...
Scripts/make_app.sh            # Debug .app（当前产出 build/帮我分析-v2.6.2.app）
Scripts/make_app.sh release    # Release .app（稳定本机签名 + Sandbox/麦克风/网络 entitlements）
open "build/帮我分析-v2.6.2.app" # 启动
Scripts/run_tests.sh           # 全部 669 个自动化用例（81 套件）
...
- 相邻 20 秒分片用 2 秒重叠原话连接本地稳定说话人组；人工标注一次后，同组历史与后续片段共用归属。单人录音还可显式选择把其余未确认发言一次标完，已人工确认给他人的内容不被覆盖。
- 人物库是唯一全局主档，本场人物只做引用。可将一人设为“我”；人工确认的背景和带原话证据的表达画像会进入后续 AI 对话与分析，录音结束后只用已确认属于“我”的原话更新画像。
- 项目对话回答不合 JSON 时只修复一次；仍失败就显示可读文本，不执行纠错或伪造来源。失败消息可原位重试，不重复追加。
- “归入业务项目”改为现有项目点选器：最近使用优先、可搜索，新建与选择分开，同名归类复用已有项目。
